...
try:
    ...
    # Fallback for different azure-search-documents versions
    AsyncAzureAISearchClient = None
...
def get_config_value(value):
    """설정 값의 문자열, 딕셔너리, 열거형 케이스를 처리하는 헬퍼 함수

    참고:
        LangGraph 설정 처리: https://langchain-ai.github.io/langgraph/concepts/configuration/
    """
...
def get_search_params(
    ...
    """지정된 검색 API가 수용하는 매개변수만 포함하도록 search_api_config 딕셔너리를 필터링합니다.

    Args:
        search_api (str): 검색 API 식별자 (예: "exa", "tavily").
        search_api_config (Optional[Dict[str, Any]]): 검색 API의 설정 딕셔너리.

    Returns:
        Dict[str, Any]: 검색 함수에 전달할 매개변수 딕셔너리.
    """
    # 각 검색 API에 대해 허용되는 매개변수 정의
    SEARCH_API_PARAMS = {
        "exa": [
            "max_characters",
            "num_results",
    ...
    # 주어진 검색 API에 대한 허용된 매개변수 목록 가져오기
    accepted_params = SEARCH_API_PARAMS.get(search_api, [])

    # 설정이 제공되지 않으면 빈 딕셔너리 반환
    if not search_api_config:
        return {}

    # 허용된 매개변수만 포함하도록 설정 필터링
...
def deduplicate_and_format_sources(
    ...
    """검색 응답 목록을 받아 읽을 수 있는 문자열로 포맷합니다.
    raw_content를 약 max_tokens_per_source 토큰으로 제한합니다.

    Args:
        search_responses: 검색 응답 딕셔너리 목록, 각각 다음을 포함:
            - query: str
        ...
        deduplication_strategy: 각 고유 URL에 대해 첫 번째 또는 마지막 검색 결과를 유지할지 여부
    Returns:
        str: 중복 제거된 소스가 포함된 포맷된 문자열
    """
    # 모든 결과 수집
    sources_list = []
...
def format_sections(sections: list[Section]) -> str:
    ...
    참고:
        LangGraph 상태 포맷팅: https://langchain-ai.github.io/langgraph/concepts/low_level/#state
    """
    formatted_str = ""
...
async def tavily_search_async(
    ...
    Returns:
            List[dict]: Tavily API의 검색 응답 목록:
                {
                    'query': str,
    ...
    # 모든 검색을 동시에 실행
    # 참고: https://docs.python.org/ko/3/library/asyncio-task.html#asyncio.gather
...
async def azureaisearch_search_async(
    ...
    """Azure AI Search API를 사용하여 동시 웹 검색 수행

    Args:
        search_queries (List[str]): 처리할 검색 쿼리 목록
        max_results (int): 각 쿼리에 대해 반환할 최대 결과 수
        topic (str): 검색을 위한 의미론적 주제 필터
        include_raw_content (bool): 원시 콘텐츠 포함 여부

    Returns:
        List[dict]: Azure AI Search API의 검색 응답 목록, 쿼리당 하나
    """
    # Azure Search 클라이언트 구성 및 생성
    # 모든 환경 변수가 설정되었는지 확인
    if not all(
        var in os.environ
        for var in [
...
def perplexity_search(search_queries):
    """Perplexity API를 사용하여 웹 검색

    Args:
        search_queries (List[SearchQuery]): 처리할 검색 쿼리 목록

    Returns:
        List[dict]: Perplexity API의 검색 응답 목록, 쿼리당 하나. 각 응답 형식:
            {
                ...
                'results': [                     # 검색 결과 목록
                    {
                        ...
                        'score': float,          # 관련성 점수
                        'raw_content': str|None  # 전체 콘텐츠 또는 보조 인용의 경우 None
                    },
                    ...
                ]
            }
        ...
        results.append(
            {
                ...
                "content": content,
                "raw_content": content,
                "score": 1.0,  # Tavily 형식과 일치하도록 점수 추가
            }
        )
            ...
            results.append(
                {
                    ...
                    "raw_content": None,
                    "score": 0.5,  # 보조 소스에는 낮은 점수
                }
            )

        # Tavily 구조와 일치하도록 응답 포맷
        search_docs.append(
            {
                "query": query,
                "follow_up_questions": None,
...
async def exa_search(
    ...
    Args:
        search_queries (List[SearchQuery]): 처리할 검색 쿼리 목록
        max_characters (int, optional): 각 결과의 원시 콘텐츠에 대해 검색할 최대 문자 수.
                                       None인 경우 text 매개변수가 객체가 아닌 True로 설정됩니다.
        num_results (int): 쿼리당 검색 결과 수. 기본값은 5입니다.
        include_domains (List[str], optional): 검색 결과에 포함할 도메인 목록.
            지정된 경우 이러한 도메인의 결과만 반환됩니다.
        exclude_domains (List[str], optional): 검색 결과에서 제외할 도메인 목록.
            include_domains와 함께 사용할 수 없습니다.
        subpages (int, optional): 결과당 검색할 하위 페이지 수. None인 경우 하위 페이지가 검색되지 않습니다.

    Returns:
        List[dict]: Exa API의 검색 응답 목록, 쿼리당 하나. 각 응답 형식:
            {
                'query': str,                    # The original search query
                'follow_up_questions': None,
                'answer': None,
    ...
    """
    # include_domains와 exclude_domains가 모두 지정되지 않았는지 확인
    if include_domains and exclude_domains:
        raise ValueError("Cannot specify both include_domains and exclude_domains")

    # Exa 클라이언트 초기화 (API 키는 .env 파일에 구성되어야 함)
    exa = Exa(api_key=f"{os.getenv('EXA_API_KEY')}")

    # 단일 쿼리를 처리하는 함수 정의
    async def process_query(query):
        # 비차단 방식으로 동기 exa 호출을 만들기 위해 run_in_executor 사용
        loop = asyncio.get_event_loop()

        # 모든 매개변수를 사용하여 executor용 함수 정의
        def exa_search_fn():
            # 매개변수 딕셔너리 구축
            kwargs = {
                # max_characters가 None이면 text를 True로 설정, 그렇지 않으면 max_characters가 있는 객체 사용
                "text": True
                if max_characters is None
                else {"max_characters": max_characters},
                "summary": True,  # EXA의 뛰어난 기능. 쿼리를 기반으로 콘텐츠의 AI 생성 요약 제공
                "num_results": num_results,
            }

            # 선택적 매개변수가 제공된 경우에만 추가
        ...
        # 예상 출력 구조와 일치하도록 응답 포맷
        formatted_results = []
        seen_urls = set()  # 중복을 피하기 위해 URL 추적

        # 항목이 dict이든 객체이든 상관없이 안전하게 값을 가져오는 헬퍼 함수
        ...
        # SearchResponse 객체에서 결과에 접근
        results_list = get_value(response, "results", [])

        # 먼저 모든 주요 결과 처리
        for result in results_list:
            # None이거나 없는 경우 기본값 0.0으로 점수 가져오기
            score = get_value(result, "score", 0.0)

            # 둘 다 사용 가능한 경우 콘텐츠에 대한 요약과 텍스트 결합
...
async def summarize_webpage(model: BaseChatModel, webpage_content: str) -> str:
    ...
    try:
        ...
        # Support cache control for Anthropic models
