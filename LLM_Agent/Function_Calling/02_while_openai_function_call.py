...
sys_message={"role": "system", "content": "너는 [내 개인 조리사 및 건강 관리사]야. 내 키, 체중, 혈당, 재료별 칼로리를 고려하여 건강당뇨식단을 추천한다."
        "너가 할 [임무] 1. 내 냉장고에 있는 식자재 목록을 알고 있다. 2. 식자재 목록을 나에게 받아서 업데이트 한다. 3. 현재 식자재 목록으로 할 수 있는 최적의 식사조리법을 가르쳐준다. 4. 기분에 따라 상황에 맞는 조리법을 권장하는데 이 때는 내가 해당 재료를 가지고 있는지 여부와 무관하게 추천한다. 5. 추천한 요리 중에서 내가 마음에 드는 요리가 생기면 거기에 필요한 재료 중 내 냉장고에 없는 재료 목록을 네이버 쇼핑에서 검색하라"}
threads.append(sys_message)
while True:
    ...
    if gpt4_answer.choices[0].message.function_call:
        ...
        if function_name == "naver":
            ...
            analysis_response = openai.chat.completions.create(
                model="gpt-4",
                messages=[
                    {"role": "system", "content": "아래 네이버 쇼핑 검색 결과를 분석하고 최적의 거래를 추천하세요. 추천하실 때는 구매할 수 있는 링크를 첨부하세요요"},
        ...
        res_txt = gpt4_answer.choices[0].message.content
        print(f"openai 답변: {res_txt}")
        threads.append({"role":"assistant","content":res_txt})